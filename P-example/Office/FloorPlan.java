...
public class FloorPlan
{
   public FloorPlan(String pathToMap)
   {
      ...
      roadSymbols = new char[] {northChar, eastChar, westChar, southChar};
      readFile(pathToMap);
   }
   ...
   {
      return symbol == northChar || symbol == eastChar || symbol == westChar || symbol == southChar;
   }
   ...
   public void registerRoom(IRoomPlan room)
   {
      ...
      for(int i = 0; i < ps.length; i++)
      {
         ...
         fillPosition(room, ps[i].line()+1, ps[i].column(), northChar, reg);
         fillPosition(room, ps[i].line()-1, ps[i].column(), southChar, reg);
         fillPosition(room, ps[i].line(), ps[i].column()+1,  westChar, reg);
         fillPosition(room, ps[i].line(), ps[i].column()-1,  eastChar, reg);
      }
   ...
   protected void fillPosition(IRoomPlan room, int line, int column, char lastPos, TemporaryPositionsRegister reg)
   {
      if (line >= 0 && line < numLines && column >= 0 && column < numColumns)
      {
         ...
         else if (map[line][column] == room.exitSymbol())
         {
            ...
            switch(lastPos)
            {
               case northChar: map[line][column] = southChar; break;
               case eastChar: map[line][column] = westChar; break;
               case westChar: map[line][column] = eastChar; break;
               case southChar: map[line][column] = northChar; break;
            }
         }
         else if (room.isRoad(map[line][column]) && (map[line][column] != room.IDSymbol()))
         {
            ...
            fillPosition(room, line+1, column, northChar, reg);
            fillPosition(room, line-1, column, southChar, reg);
   ...
   public final static char northChar = 'n';
   public final static char  eastChar = 'e';
